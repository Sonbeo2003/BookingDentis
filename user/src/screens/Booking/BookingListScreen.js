  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      try {
        ...
        if (!userData) {
          console.warn("Không tìm thấy dữ liệu người dùng, chuyển hướng đến trang đăng nhập");
...
const fetchPendingBookings = async (user_id) => {
  ...
  const fetchInProgressBookings = async (user_id) => {
    try {
      ...
      Alert.alert("Lỗi", "Không thể tải lịch hẹn đang thực hiện. Vui lòng thử lại.");
      return [];
    }
  };
  ...
  const fetchCompletedBookings = async (user_id) => {
    try {
      ...
      Alert.alert("Lỗi", "Không thể tải lịch hẹn hoàn thành. Vui lòng thử lại.");
      return [];
    }
  };
  ...
  const fetchPaidBookings = async (user_id) => {
    try {
      ...
      Alert.alert("Lỗi", "Không thể tải lịch hẹn đã thanh toán. Vui lòng thử lại.");
      return [];
    }
  };
  ...
  const handleCancel = async (reason) => {
    ...
    try {
      const response = await axios.post(`${url}/api_doctor/cancel_booking.php`, {
      ...
      if (response.data.success) {
        ...
        Alert.alert("Thông báo", response.data.message || "Có lỗi khi hủy lịch hẹn.");
      }
    } catch (error) {
  ...
  const BookingTab = ({ fetchBookings, showCancelButton, showPaymentButton }) => {
      ...
      <View style={styles.tabContainer}>
        {bookings && bookings.length > 0 ? (
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.bookingCard}>
                <View style={styles.bookingInfo}>
                  ...
                  <Text style={styles.text}>Tên dịch vụ: {item.services || "Cuộc hẹn y tế"}</Text>
                </View>
                <View style={styles.bookingInfo}>
                  <Icon name="map-marker" size={20} color="#fff" />
                  <Text style={styles.text}>Địa điểm: {item.clinic_name || item.name || "Chưa xác định"}</Text>
                </View>
                <View style={styles.bookingInfo}>
                  <Icon name="user-md" size={20} color="#fff" />
                  <Text style={styles.text}>Bác sĩ: {item.doctor_name || item.full_name || "Chưa xác định"}</Text>
                </View>
                <View style={styles.bookingInfo}>
                  <Icon name="calendar" size={20} color="#fff" />
                  <Text style={styles.text}>Ngày hẹn: {item.appointment_date || "Chưa xác định"}</Text>
                </View>
                <View style={styles.bookingInfo}>
                  <Icon name="clock-o" size={20} color="#fff" />
                  <Text style={styles.text}>Thời gian: {item.appointment_time || "Chưa xác định"}</Text>
                </View>
                <View style={styles.bookingInfo}>
                  <Icon name="info-circle" size={20} color="#fff" />
                  <Text style={styles.text}>Trạng thái: {statuses[item.status] || "Chưa xác định"}</Text>
                </View>
                {item.services && item.services.length > 0 && (
                  <View style={styles.bookingInfo}>
                    <Icon name="cogs" size={20} color="#fff" />
                    <Text style={styles.text}>Dịch vụ: {item.services}</Text>
                  </View>
                )}
                {item.note && (
                  <View style={styles.bookingInfo}>
            ...
            keyExtractor={(item) => (item.appointment_id ? item.appointment_id.toString() : Math.random().toString())}
  ...
  if (!userId) {
    return (
      <View style={styles.noBookingsContainer}>
        ...
        <Text style={styles.noBookingsText}>Bạn cần đăng nhập để xem lịch hẹn.</Text>
        <TouchableOpacity
